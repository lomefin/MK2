...
class ConfigsHandler(mkhandler.MKGAEHandler):
	...
	def internal_get(self):
		...
		config_data = MKConfig.all().order('-creation_date')
		values = {
			'config_data':config_data,
		}
		
		self.render('list_configs',template_values=values)
	
	
	def internal_post(self):
		config = MKConfig()
		config.name = self.request.get('config_name')
		config.value = self.request.get('config_value')
		config.put()
		self.redirect('/admin/configs/')


def main():
  application = webapp.WSGIApplication([('/admin/configs/', ConfigsHandler),
										],
                                       debug=True)
