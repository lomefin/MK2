...
class WeekModeProperty(db.Property):
	data_type = int
	values = ['online','offline','vacation','inactive']
	
	def validate(self, value):
		value = super(WeekModeProperty,self).validate(value)
		if value is not None and value not in self.values:
			raise datastore_errors.BadValueError(
				"Property can have only values '%s', '%s', '%s', or '%s'" % (self.values[0],self.values[1],self.values[2],self.values[3]))
		return value
